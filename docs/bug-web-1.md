...
1. The whole screen is replaced by a bare "Loading…" state and rebuilt from scratch for
   every realtime event, even ones irrelevant to what's on screen (e.g. the opponent
   dragging a bench player).
...
- The refresh was triggered by something unrelated to this lineup's batting order
  (opponent's lineup, sim status, etc.) → the local draft should survive untouched.
...
- Start editing the batting order in one tab (don't let the 500ms auto-save fire), then in
  a second tab change the *opponent's* SP or batting order. Confirm the first tab's
  in-progress edit and drag state survive and the page doesn't flash to a loading state.
- Start editing the batting order in one tab, then save a different batting order for the
