...
- Multi-out plays (double and triple plays): the sim currently treats every batter out as exactly one out, and base runners neither advance nor are retired on a groundout (`go` resolution in the sim engine). A realistic model would, with runners on base, sometimes convert a groundout into a double or triple play (retiring a lead runner in addition to — or instead of — the batter), and would otherwise advance a runner who is not retired (e.g. a runner on first moving to second on a groundout). The `sim_event_runner_outcomes` schema already supports multiple putouts per event (`putout_type`, `putout_at_base`), so this is an engine change, not a schema migration. Note the extra-innings termination guarantee deliberately does not depend on the number of outs, so it remains sound if this is implemented (see Architecture → Simulation Design → Extra innings).
